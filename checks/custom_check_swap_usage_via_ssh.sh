...
## custom_check_swap_usage_via_ssh.sh
## version 0.1
## $1 is ssh username
## $2 is ssh address
##
...
## chmod +x custom_check_for_swap_usage_via_ssh.sh
## add following four lines to /usr/local/nagios/etc/objects/commands.cfg
## define command {
##        command_name    custom_check_swap_usage_via_ssh
##        command_line    $USER1$/custom_check_swap_usage_via_ssh.sh $ARG1$ $HOSTADDRESS$   ## ARG1 is ssh username
##}
##
##
...
##        check_command                   custom_check_swap_usage_via_ssh!user
...
sshOutput=$(ssh "$1"@"$2" -o ConnectTimeout=10 -o BatchMode=yes "vmstat" | awk '(NR==2){for(i=1;i<=NF;i++)if($i=="si"){getline; print $i}}')
sshError=$( (ssh "$1"@"$2" -o ConnectTimeout=10 -o BatchMode=yes "vmstat" | grep si > /dev/null 2>&1 ))

## Thanks to Michał Šrajer for the above line, parsing vmstat
...
if [ -z "$sshOutput" ] ; then  ## If vmstat and AWK returned a null value, try just running vmstat alone
        ...
        if [ "$sshOutput" -lt 200 ]; then
                echo "OK - Only swapping $sshOutput bits right now"
                exit 0
        else
                if [ "$sshOutput" -gt 600 ]; then
                        echo "CRITICAL - Currently swapping $sshOutput bits to disk because it's out of RAM"
                        exit 2
                else
                        echo "WARNING - Currently swapping $sshOutput bits to disk because it's low on RAM. Reduce load or increase resources before this becomes a problem"
                        exit 1
                fi
        fi
else
