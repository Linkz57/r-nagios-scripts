...
## custom_check_freenas_updates_via_ssh.sh
## version 1.0
## $1 is ssh username
## $2 is ssh address
##
...
##        command_line    $USER1$/custom_check_foo.sh $ARG1$ $ARG2$    ## ARG1 is ssh username, ARG2 is server address
##}
##
##
...
sshOutput=$(ssh "$1"@"$2" -o ConnectTimeout=10 -o BatchMode=yes "sh -c 'freenas-update check 2>&1'")



if echo "$sshOutput" | grep "No updates available" > /dev/null ; then
