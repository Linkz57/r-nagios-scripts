...
## check_running_process_via_ssh.sh
## version 1.0
## $1 is ssh username
## $2 is ssh address
...
## save check_running_process_via_ssh.sh to /usr/local/nagios/libexec
## chmod +x check_running_process_via_ssh.sh
## add following four lines to /usr/local/nagios/etc/objects/commands.cfg
## define command {
##        command_name    check_running_process_via_ssh
##        command_line    $USER1$/check_running_process_via_ssh.sh $ARG1$ $ARG2$ $ARG3$   ## ARG1 is ssh username, ARG2 is storage server address, ARG3 is the process name to check.
##}
##
##
...
##        check_command                   check_running_process_via_ssh!user!127.000.000.001!mahprocess
...
sshOutput=$(ssh $1@$2 -o ConnectTimeout=10 -o BatchMode=yes "ps aux | grep -v grep")
...
sshError=$( (ssh $1@$2 -o ConnectTimeout=10 -o BatchMode=yes "ps aux | grep -v grep" > /dev/null) 2>&1 )


if [ -z "$sshError" ] ; then
        if echo $sshOutput | grep $3 > /dev/null ; then
                echo "OK - Service '$3' is currently running at $2"
                exit 0
        else
